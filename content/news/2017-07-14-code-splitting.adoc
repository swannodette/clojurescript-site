...
match user expectations. However if we left it at that we would be missing
a huge opportunity. Along with dead code elimination Google Closure employs
another useful optimization - *cross module code motion*. Individual program values
(including functions and methods) which are free of side effects can be moved
*back down* the module graph.

So some if some single function present in `:cljs-base` is only ever used in
`:module-a` will be moved back to `:module-a`.
